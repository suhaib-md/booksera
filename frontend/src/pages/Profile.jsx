...
function Profile() {
  ...
  const handleUpdate = () => {
    axios.post("http://localhost:8000/api/profile/update/", user, { withCredentials: true })
  ...
  const handleImageUpload = async (e) => {
    ...
    try {
      const response = await axios.post("http://localhost:8000/api/profile/upload/", formData, {
    ...
    <div className="p-6 max-w-md mx-auto bg-white shadow-lg rounded-lg">
      ...
      <div className="flex flex-col items-center my-4">
        {user.profile_picture ? (
          <img src={user.profile_picture} alt="Profile" className="w-24 h-24 rounded-full shadow-md" />
        ) : (
      ...
      <div className="space-y-3">
        ...
        <textarea 
          ...
          onChange={(e) => setUser({ ...user, books_read: e.target.value.split(", ") })}
